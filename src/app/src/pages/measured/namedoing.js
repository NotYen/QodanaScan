...
import playfootball from '../../assecs/images/source/source_58.png';
import readbook from '../../assecs/images/source/source_20.png';
...
const ConformMeasured = () => {
            ...
            recorder.addEventListener('stop', async () => {
                ...
                await axios.post(`${process.env.REACT_APP_API_URL}/api/file/upload/audio`, formData, { headers: { 'Content-Type': 'multipart/form-data' } })
                    .then(resource => {
                        ...
                        if (isAnswer === 'true') {
                            if (key !== 'namedoing-playfootball') {
                                setCompleted(true);
                                setTimeout(() => navigate('/games/namedoing'), 1000);
                            } else {
                                actions.setModel('gold', () => {
                                    navigate('/games/namedoing');
                                });
                            }
    ...
    return (
        <Box layerStyle={ 'doing' }>
            <Subject title='描述動作' illustrate={ instruction.illustrate } record={handleRecord} />
            <Box className="measured-container">
                <div className="answer-box flex-center">
                    <samp className="samp-first">
                        <LazyLoadImage src={ handwashing } />
                        { key === 'namedoing-handwashing' && <Image className='finger-handwashing' src={ finger } /> }
                    </samp>
                    <samp>
                        <LazyLoadImage src={ playfootball } />
                        { key === 'namedoing-playfootball' && <Image className='finger-playfootball' src={ finger } /> }
                    </samp>
                    <samp>
                        <LazyLoadImage src={ readbook } />
                        { key === 'namedoing-readbook' && <Image className='finger-readbook' src={ finger } /> }
                    </samp>
                </div>
