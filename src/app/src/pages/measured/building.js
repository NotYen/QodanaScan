...
import scope_house from '../../assecs/images/icon/scope_build-house.png';
import scope_own from '../../assecs/images/icon/scope_build-own.png';
import scope_train from '../../assecs/images/icon/scope_build-train.png';
import scope_take from '../../assecs/images/icon/scope_build-take.png';
import audio from '../../assecs/images/icon/audio.png';
import videoBG_house from '../../assecs/images/gif/building-house.gif';
import videoBG_own from '../../assecs/images/gif/building-own.gif';
import videoBG_train from '../../assecs/images/gif/building-train.gif';
import videoBG_take from '../../assecs/images/gif/building-take.gif';
...
const BuildingMeasured = () => {
    ...
    const [palmtext, setPalmText] = useState(instruction.illustrate.prompt);
    const [palmtext_2, setPalmText_2] = useState(instruction.illustrate.prompt_2);
    const style = palmtext_2 ? { paddingBottom: '0' } : {};
    ...
    const handleRecordClick = () => {
        ...
        setPalmText(instruction.illustrate.prompt);
        setPalmText_2(instruction.illustrate.prompt_2);
    };
    ...
    const handleUpdate = async () => {
        ...
        formData.append('question', 'block');
    ...
    useEffect(() => {
        console.log(instruction.illustrate);
    ...
    useEffect(() => {
        if (key === 'building-house') {
            setScope(scope_house);
            setVideoBG(videoBG_house);
        } else if (key === 'building-own') {
            setScope(scope_own);
            setVideoBG(videoBG_own);
        } else if (key === 'building-train') {
            setScope(scope_train);
            setVideoBG(videoBG_train);
        } else if (key === 'building-take') {
            setScope(scope_take);
            setVideoBG(videoBG_take);
        } else {
            setScope(scope_house);
            setVideoBG(videoBG_house);
        }
    ...
    return (
        <Stack className="position-fixed" layerStyle={ 'building' }>
            <Box className="camera-container">
                <Text className="title">積木任務</Text>
            ...
            <Stack className="control-container">
                <p className="prompt-text flex-center" style={style} >
                    {
                        isRecording === 'stoping' &&
                        <samp className="icon">
                ...
                <p className="prompt-text flex-center">{ palmtext_2 }</p>
