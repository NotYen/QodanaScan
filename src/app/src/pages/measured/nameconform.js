...
const ConformMeasured = () => {
            ...
            recorder.addEventListener('stop', async () => {
                ...
                await axios.post(`${process.env.REACT_APP_API_URL}/api/file/upload/audio`, formData, { headers: { 'Content-Type': 'multipart/form-data' } })
                    .then(resource => {
                        ...
                        if (isAnswer === 'true') {
                            setCompleted(true);
                            setTimeout(() => navigate('/game/nameconform'), 1000);
                        ...
                        } else navigate('/games/nameconform');
    ...
    return (
        <Box layerStyle={ 'conform' }>
            <Subject title='圖卡對應' illustrate={ instruction.illustrate } record={handleRecord} />
            <Box className="measured-container">
                <div className="answer-box flex-center">
                    <samp>
                        <LazyLoadImage src={ train } />
                        { key === 'nameconform-train' && <Image className='finger-train' src={ finger } /> }
                    </samp>
                    <samp>
                        <LazyLoadImage src={ clothing } />
                        { key === 'nameconform-clothing' && <Image className='finger-clothing' src={ finger } /> }
                    </samp>
                    <samp>
                        <LazyLoadImage src={ pen } />
                        { key === 'nameconform-pen' && <Image className='finger-pen' src={ finger } /> }
                    </samp>
                    <samp>
                        <LazyLoadImage src={ grape } />
                        { key === 'nameconform-grape' && <Image className='finger-grape' src={ finger } /> }
                    </samp>
                    <samp>
                        <LazyLoadImage src={ chair } />
                        { key === 'nameconform-chair' && <Image className='finger-chair' src={ finger } /> }
                    </samp>
                    <samp>
                        <LazyLoadImage src={ airplane } />
                        { key === 'nameconform-airplane' && <Image className='finger-airplane' src={ finger } /> }
                    </samp>
                </div>
