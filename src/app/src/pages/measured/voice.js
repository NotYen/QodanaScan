...
import banana from '../../assecs/images/source/source_17.png';
import ballon from '../../assecs/images/source/source_18.png';
...
const IdentifyMeasured = () => {
            ...
            recorder.addEventListener('stop', async () => {
                ...
                await axios.post(`${process.env.REACT_APP_API_URL}/api/file/upload/audio`, formData, { headers: { 'Content-Type': 'multipart/form-data' } })
                    .then(resource => {
                        ...
                        if (isAnswer === 'true') {
                            setCompleted(true);
                            setTimeout(() => navigate('/games/voice'), 1000);
    ...
    return (
        <Box layerStyle={'identify'}>
            <Subject title='圖卡命名' illustrate={instruction.illustrate} record={handleRecord} />
            <Box className="measured-container">
                <div className="answer-box flex-center">
                    <samp>
                        <LazyLoadImage src={car} />
                        {key === 'voice-car' && <Image className='finger-car' src={finger} />}
                    </samp>
                    <samp>
                        <LazyLoadImage src={banana} />
                        {key === 'voice-banana' && <Image className='finger-banana' src={finger} />}
                    </samp>
                    <samp>
                        <LazyLoadImage src={ballon} />
                        {key === 'voice-ballon' && <Image className='finger-ballon' src={finger} />}
                    </samp>
                    <samp>
                        <LazyLoadImage src={puppy} />
                        {key === 'voice-puppy' && <Image className='finger-puppy' src={finger} />}
                    </samp>
                </div>
