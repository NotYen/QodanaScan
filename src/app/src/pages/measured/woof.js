...
/* import `Image` source */
import puppy from '../../assecs/images/source/source_66.png';
...
const IdentifyMeasured = () => {
            ...
            recorder.addEventListener('stop', async () => {
                ...
                await axios.post(`${process.env.REACT_APP_API_URL}/api/file/upload/audio`, formData, { headers: { 'Content-Type': 'multipart/form-data' } })
                    .then(resource => {
                        ...
                        if (isAnswer === 'true') {
                            setCompleted(true);
                            setTimeout(() => navigate('/games/woof'), 1000);

                            dispatch(setCorrect(true));

                        } else navigate('/games/woof');
    ...
    return (
        <Box layerStyle={'woof'}>
            <Subject title='學動物叫' illustrate={instruction.illustrate} record={handleRecord} />
            <Box className="measured-container">
                <div className="answer-box flex-center">
                    <samp className="img-puppy">
